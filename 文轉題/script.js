class QuizManager {
  constructor() {
    ...
    // 添加載入檔案按鈕的事件監聽器
    document.getElementById('loadDriveFiles').addEventListener('click', () => {
      if (!gapi.client) {
    ...
    // 設定 Github repository 相關資訊
    this.githubRepo = {
      owner: 'puti001',
      repo: 'public',
      path: 'texts/3rd_Chinese' // 文字檔所在的資料夾路徑
    };
    
    // 初始化時載入文字檔列表
    this.loadGithubTextFiles();
  }
  ...
  initializeElements() {
    ...
    this.loadDriveBtn = document.getElementById('loadDrive');
    this.driveFileList = document.getElementById('driveFileList');
  ...
  addEventListeners() {
    ...
    // Add Google Drive listeners
    this.loadDriveBtn.addEventListener('click', () => this.loadDriveFiles());
  }
...
${text}`;
  ...
  // 讀取 Github 資料夾中的文字檔列表
  async loadGithubTextFiles() {
    try {
      const response = await fetch(
        `https://api.github.com/repos/${this.githubRepo.owner}/${this.githubRepo.repo}/contents/${this.githubRepo.path}`
      );
      
      if (!response.ok) {
        throw new Error('無法讀取 Github 資料夾');
      }
      ...
      textFiles.forEach(file => {
        const button = document.createElement('button');
        button.className = 'file-button';
        button.textContent = file.name.replace('.txt', '');
        button.addEventListener('click', () => this.loadGithubTextContent(file.download_url));
        this.fileList.appendChild(button);
      });
    } catch (error) {
      console.error('Error loading Github files:', error);
      this.fileList.innerHTML = '<p style="color: red;">讀取檔案列表失敗</p>';
    }
  }

  // 讀取選定的文字檔內容
  async loadGithubTextContent(fileUrl) {
    try {
      const response = await fetch(fileUrl);
      if (!response.ok) {
        throw new Error('無法讀取文字檔');
      }
      ...
      const selectedBtn = Array.from(this.fileList.children)
        .find(btn => btn.textContent === fileUrl.split('/').pop().replace('.txt', ''));
      if (selectedBtn) {
        selectedBtn.classList.add('active');
      }
    } catch (error) {
